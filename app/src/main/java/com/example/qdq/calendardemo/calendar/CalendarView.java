...
import com.example.qdq.calendardemo.R;
...
public class CalendarView extends ViewPager {
    ...
    private int[] dateStart;//日历的开始年、月、日
    private int[] dateEnd;//日历的结束年、月、日
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        if (dateStart == null) {
            dateStart = new int[]{1900, 1, 1};
        }
        dateEnd = CalendarUtil.strToArray(dateEndStr);
        if (dateEnd == null) {
            dateEnd = new int[]{2049, 12, 31};
        }
